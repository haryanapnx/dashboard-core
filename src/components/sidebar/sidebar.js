...
class Sidebar extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			homepages: this.activeRoute("/home") !== "" ? true : false,
			ordersPages: this.activeRoute("/orders") !== "" ? true : false,
	...
	/*--------------------------------------------------------------------------------*/
	activeRoute=(routeName)=> { return '/orders'.indexOf(routeName) > -1 ? 'selected' : ''; }
					...
					<PerfectScrollbar className="sidebar-nav">
						...
						<Nav id="sidebarnav">
							{this.props.routes.map((prop, key) => {
								if (prop.redirect || prop.hide_menu) {
									return null;
								}
									...
									return (
										...
										<li className={this.activeRoute(prop.path) + ' sidebar-item'} key={key}>
											<a href=' ' data-toggle="collapse" className="sidebar-link has-arrow" aria-expanded={this.state[prop.state]} onClick={() => this.setState(firstdd)} >
												...
												<ul className="first-level">
													{prop.child.map((prop, key) => {
														...
														if (prop.collapse) {
															...
															return (
																<li className={this.activeRoute(prop.path) + ' sidebar-item'} key={key} >
																	<a href=' ' data-toggle="collapse" className="sidebar-link has-arrow" aria-expanded={this.state[prop.state]} onClick={() => this.setState(seconddd)} >
																		<i className={prop.icon} />
																		<span className="hide-menu">{prop.name}</span>
																	</a>
																	{/*--------------------------------------------------------------------------------*/}
